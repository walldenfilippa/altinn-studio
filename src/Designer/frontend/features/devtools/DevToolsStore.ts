...
export const DevToolsStore = createStore<Store>((set) => ({
  ...
  actions: {
    ...
    layoutInspectorSet: (selectedComponentId: string | undefined) =>
      set({ layoutInspector: { selectedComponentId } }),
    nodeInspectorSet: (selectedNodeId: string | undefined) =>
      set({ nodeInspector: { selectedNodeId } }),
    postLogs: (newLogs: IDevToolsLog[]) => set(({ logs }) => ({ logs: [...logs, ...newLogs] })),
    logsClear: () => set({ logs: [] }),
  },
}));

export const useDevToolsStore = <T>(selector: (state: Store) => T) =>
  useStore(DevToolsStore, selector);
